...
- `private`を付けるとそのクラス内だけから、 `protected` を付けると派生クラスからのみアクセスできるメソッドになる
- `private[this]` をつけると、同じオブジェクトからのみアクセス可能になる.
  - また、 `private[パッケージ名]` を付けると同一パッケージに所属しているものからのみ
...
- トレイとの大きな用途の一つは、クラスがすでに持っているメソッドを使って自動的にメソッドを増やすと言うものである.
