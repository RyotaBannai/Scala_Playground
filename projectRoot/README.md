...
- `Collection`:
  ...
  - シーケンストレイト(Seq):
    ...
    - `バッファー`:
      - `+=` `++=` 末尾に要素を追加. これらの同じ目的のメソッドが複数ある理由は、Traversable などの上位トレイトから継承しているため. また、Traversable で定義されているメソッドの引数は Traversable であるが、その下層レイアーで定義されたメソッドの引数はそのレイアーの型になることがあるので注意(例えば、集合の `++` メソッドと `union`または`|`メソッド)(SP470)
      - `+=:` `++=:` 先頭に要素を追加
      - insert insertAll 要素を挿入
      - remove, `-=` 要素を削除
      - add/remove は演算結果が Boolean であるため結果が欲しい時に使う(SP473)
      - 集合サイズが4 以下の場合は、イミュータブル版を利用する(SP473)
